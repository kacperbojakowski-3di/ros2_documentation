...
A hands-on exercise lets you practice viewing 3D data in RViz, without needing a robot.
...
The ``Marker`` display can show shapes that your node publishes as ``visualization_msgs/msg/Marker`` messages.
You do not need RViz to know the meaning of that data ahead of time.

This tutorial uses a small publisher that cycles through a cube, sphere, arrow, and cylinder.
You use that publisher only as sample data, so you can practise setup, viewing, and camera control in RViz.
...
#. ROS is installed.
   For more information, see :doc:`Installation options <../../../../Get-Started/Installation>`.
#. You can source your setup files.
   For more information, see :doc:`Setting up your environment <../../../../Get-Started/Configuring-ROS2-Environment>`.
#. You have a workspace you can build with ``colcon``.
   For more information, see :doc:`Creating a workspace <../../../../ROS-Framework/client-libraries/Working-with-Client-Libraries/Creating-A-Workspace/Creating-A-Workspace>`.
...
Clone the repository into the ``src`` directory of your workspace, then build only this package.
Use the branch that matches this documentation set:
...
Open a **new** terminal, source ROS and your workspace overlay, then run the node:
...
For marker fields and other marker types, see :doc:`Marker: Display types <../Marker-Display-types/Marker-Display-types>`.
...
**Global Status** may show a warning because this tutorial does not publish transforms.
That is expected.
...
This tutorial does not publish transforms, so RViz must use that same frame.
...
**Global Status** can stay at Warn because this tutorial still has no transforms.
If the Marker status is a warning or error, expand the Marker entry and check the topic name and the fixed frame.

6 Change the view
^^^^^^^^^^^^^^^^^

The Views panel selects how RViz controls the camera and how it projects the 3D scene.
...
To change the view, open the **Type** list in the Views panel and select a different type:

* **Orbit**: Rotate around a focal point.
  This is the default and the one used in the next step.
* **FPS**: First-person control, as if you look around from the camera position.
* **TopDownOrtho**: Look down the fixed-frame Z axis.
  Objects do not get smaller with distance.

You can save a named view with **Save** on the Views panel.
A saved view stores the camera type, pose, and target frame.
...
* The left button, right button, and scroll wheel keep the focal point and change the camera pose or zoom.
* The middle button, or anything with **Shift**, moves the focal point.
...
Why do I set the fixed frame to ``my_frame``?
   ...
   This tutorial does not provide transforms, so RViz must use that same frame as **Fixed Frame**.

Why is the 3D view black or empty?
   RViz has nothing to draw until a display is configured for data that exists.
   Start ``basic_shapes``, set **Fixed Frame** to ``my_frame``, and add a Marker display on ``/visualization_marker``.
   Leave **Fixed Frame** as ``map`` and the marker will not appear.

How do I show the marker shapes?
   Run ``basic_shapes``, start RViz, set the fixed frame to ``my_frame``, and add a Marker display.
   The default Marker topic is ``/visualization_marker``.
